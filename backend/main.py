...
def log_callback(printer: Printer, print_job: PrintJob):
    assert printer.id is not None and print_job.id is not None
    log = Log(
...
async def add_printjob(request: Request, print_job: PrintJob):
    ...
    least_busy_printer = min(
        printers, key=lambda printer: len(printer._printing_queue)
    )
    ...
    print_job.id = (int(datetime.now().timestamp() * 1000) << 32) | least_busy_printer.id
    least_busy_printer.add_printjob(print_job)

    return {"printer_id": least_busy_printer.id, "print_job_id": 1}
